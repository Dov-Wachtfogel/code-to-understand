...
class SignatureDataGenerator(object):
    ...
    def __init__(self, dataset, tot_writers, num_train_writers, num_valid_writers,
        ...
        self.image_dir = '/home/' + 'dov-wachtfogel' + '/sing_dataset/dataset_signature_final/Dataset/' + dataset + '/'
        ...
        idx_valid_writers = sorted(
            np.random.choice([x for x in idx_writers if x not in idx_train_writers], num_valid_writers, replace=False))
        idx_test_writers = sorted(
            np.random.choice([x for x in idx_writers if x not in idx_train_writers and x not in idx_valid_writers],
                             num_test_writers, replace=False))
